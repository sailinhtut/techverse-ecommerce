...
@section('admin_dashboard_content')
    <div class="p-5 min-h-screen">
        ...
        <div class="mt-3 flex xl:flex-row flex-col justify-between gap-2">
            ...
            <dialog id="bulk_delete_selected" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <p class="py-2 mb-0 text-sm">
                        ...
                        <span class="italic text-error">Selected Articles</span>?
                    </p>
            ...
            <dialog id="bulk_delete_all" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <p class="text-sm mb-4">
                        Are you sure you want to delete
                        <span class="text-error">All Articles</span>?
                    </p>
            ...
            <div class="flex flex-row flex-wrap justify-start xl:justify-end gap-2">
                ...
                <form id="queryForm" method="GET" action="{{ request()->url() }}" class="join join-horizontal">
                    <template x-data x-if="$store.article_search_setting.sortBy">
                        <input type="hidden" name="sortBy" :value="$store.article_search_setting.sortBy">
                    </template>

                    <template x-data x-if="$store.article_search_setting.perPage">
                        <input type="hidden" x-data name="perPage" :value="$store.article_search_setting.perPage">
                    </template>

                    <template x-data x-if="$store.article_search_setting.orderBy">
                        <input type="hidden" x-data name="orderBy" :value="$store.article_search_setting.orderBy">
                    </template>

                    <input type="text" x-data x-cloak class="join-item input input-sm rounded-l-box" name="query"
                        :value="$store.article_search_setting.query"
                        @change="$store.article_search_setting.query = $event.target.value; $store.article_search_setting.save(); $el.form.submit()">
                ...
                <form method="GET" action="{{ request()->url() }}">
                    <template x-data
                        x-if="$store.article_search_setting.query && $store.article_search_setting.query.length > 0">
                        <input type="hidden" name="query" :value="$store.article_search_setting.query">
                    </template>

                    <template x-data x-if="$store.article_search_setting.sortBy">
                        <input type="hidden" x-data name="sortBy" :value="$store.article_search_setting.sortBy">
                    </template>
                    ...
                    <select name="perPage" x-data x-cloak class="select select-sm w-fit shrink-0" x-data
                        x-model="$store.article_search_setting.perPage"
                        @change="$store.article_search_setting.perPage = $event.target.value; $store.article_search_setting.save(); $el.form.submit()">
                ...
                <form method="GET" action="{{ request()->url() }}">
                    ...
                    <select name="orderBy" x-data x-cloak x-model="$store.article_search_setting.orderBy"
                        class="select select-sm w-fit shrink-0" x-data :value="$store.article_search_setting.orderBy"
                        @change="$store.article_search_setting.orderBy = $event.target.value; $store.article_search_setting.save() ; $el.form.submit()">
                        <option value="desc">
                ...
                <form method="GET" action="{{ request()->url() }}">
                    ...
                    <select name="sortBy" class="select select-sm w-fit shrink-0" x-data x-cloak
                        :value="$store.article_search_setting.sortBy"
                        @change="$store.article_search_setting.sortBy = $event.target.value; $store.article_search_setting.save() ; $el.form.submit()">
                ...
                {{-- <button class="btn btn-sm bg-base-100 font-normal shadow-none border-slate-300" x-data x-transition x-cloak
                    @click="$store.article_search_setting.showFilterOption = !$store.article_search_setting.showFilterOption;$store.article_search_setting.save()">
                    <span x-text="$store.article_search_setting.showFilterOption ? 'Hide Filter' : 'Filter Option'"></span>
                </button> --}}
                <button class="btn btn-sm bg-base-100 font-normal shadow-none border-slate-300" x-data x-transition x-cloak
                    @click="$store.article_search_setting.showDisplayOption = !$store.article_search_setting.showDisplayOption;$store.article_search_setting.save()">
                    <span
                        x-text="$store.article_search_setting.showDisplayOption ? 'Hide Display' : 'Display Option'"></span>
                </button>
            </div>
        </div>
        ...
        <div class="mt-3 p-3 border border-base-300 rounded-md flex flex-col gap-2" x-cloak x-transition x-data
            x-show="$store.article_search_setting.showFilterOption">
            <p class="text-xs">Filter Options</p>
            <div class="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
                {{-- <div class="flex flex-col gap-1">
                    <select x-data class="select select-sm text-xs" name="brand"
                        :value="$store.article_search_setting.pinnedFilter ? 'true' : 'false'"
                        @change="$store.article_search_setting.pinnedFilter = $event.target.value; $store.article_search_setting.save()">
            ...
            <div class="flex flex-row gap-2">
                ...
                <button class="btn btn-sm" x-data @click="$store.article_search_setting.resetFilter()">Reset</button>
            </div>
        </div>

        {{-- column displaying --}}
        <div class="mt-3 p-3 border border-base-300 rounded-md flex flex-col gap-2" x-cloak x-transition x-data
            x-show="$store.article_search_setting.showDisplayOption">
            ...
            <div class="flex sm:flex-row flex-col flex-wrap gap-3">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" class="checkbox checkbox-xs rounded-sm" x-data
                        :checked="$store.article_search_setting.showIDColumn"
                        @change="$store.article_search_setting.showIDColumn = !$store.article_search_setting.showIDColumn; $store.article_search_setting.save()">
                    <span class="text-xs">Show ID</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" class="checkbox checkbox-xs rounded-sm" x-data
                        :checked="$store.article_search_setting.showUpdatedTimeColumn || $store.article_search_setting
                            .is_last_updated_filter"
                        @change="$store.article_search_setting.showUpdatedTimeColumn = !$store.article_search_setting.showUpdatedTimeColumn; $store.article_search_setting.save()">
                    <span class="text-xs">Show Updated Time</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" class="checkbox checkbox-xs rounded-sm" x-data
                        :checked="$store.article_search_setting.showCreatedTimeColumn || $store.article_search_setting
                            .is_last_created_filter"
                        @change="$store.article_search_setting.showCreatedTimeColumn = !$store.article_search_setting.showCreatedTimeColumn; $store.article_search_setting.save()">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th x-cloak x-data x-show="$store.article_search_setting.showIDColumn" class="w-[50px]">ID
                            </th>
                            <th>Title</th>
                            <th>Description</th>
                            <th>Status</th>
                            <th>Featured</th>
                            <th x-cloak x-data
                                x-show="$store.article_search_setting.showUpdatedTimeColumn || $store.article_search_setting.is_last_updated_filter">
                                Updated At</th>
                            <th x-cloak x-data
                                x-show="$store.article_search_setting.showCreatedTimeColumn || $store.article_search_setting.is_last_created_filter">
                                Created At</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($articles as $article)
                            <tr>
                                <td>
                                    <input type="checkbox" class="checkbox checkbox-xs rounded-sm row-checkbox"
                                        data-id="{{ $article['id'] }}" x-data
                                        :checked="$store.bulk.candidates.includes({{ $article['id'] }})"
                                        @change="$store.bulk.toggleCandidate({{ $article['id'] }})">
                                </td>
                                <td>{{ $loop->iteration + ($articles->currentPage() - 1) * $articles->perPage() }}</td>
                                <td x-cloak x-data x-show="$store.article_search_setting.showIDColumn">
                                    {{ $article['id'] }}
                                </td>
                                <td>
                                ...
                                <td class="max-w-[200px] truncate">{{ $article['description'] ?? '-' }}</td>
                                <td>{{ ucfirst($article['status'] ?? '-') }}</td>
                                <td>{{ ucfirst($article['is_featured'] ? 'Normal' : 'Featured') }}</td>
                                <td x-cloak x-data
                                    x-show="$store.article_search_setting.showUpdatedTimeColumn || $store.article_search_setting.is_last_updated_filter">
                                    {{ $article['updated_at'] ? \Carbon\Carbon::parse($article['updated_at'])->format('Y-m-d h:i A') : '-' }}
                                </td>
                                <td x-cloak x-data
                                    x-show="$store.article_search_setting.showCreatedTimeColumn || $store.article_search_setting.is_last_created_filter">
                                    ...
                                    <div tabindex="0" role="button" class="dropdown dropdown-left">
                                        ...
                                        <ul tabindex="0"
                                            ...
                                            <li><button class="text-error"
                                                    onclick="document.getElementById('delete_modal_{{ $article['id'] }}').showModal()">Delete</button>
                                            </li>
                                        </ul>
                                    </div>
                                </td>
                            ...
                            <!-- Delete Modal -->
                            <dialog id="delete_modal_{{ $article['id'] }}" class="modal">
                                <div class="modal-box">
                                    <form method="dialog">
                                        <button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</button>
                                    </form>
                                    <p class="text-lg font-semibold">Confirm Delete</p>
                                    <p class="text-sm mb-4">Are you sure you want to delete this article <span
                                            class="text-error">{{ $article['title'] }}</span>?</p>
                                    <div class="modal-action">
...
@push('script')
    <script>
        document.addEventListener('alpine:init', function() {
            ...
            Alpine.store('article_search_setting', {
                ...
                init() {
                    const savedSetting = JSON.parse(localStorage.getItem('article_search_setting') ?? "{}");
                ...
                save() {
                    ...
                    localStorage.setItem('article_search_setting', JSON.stringify(data));
                },
            ...
            Alpine.store('article_search_setting').init();
        });
    </script>
@endpush
