...
@section('admin_dashboard_content')
    <div class="p-3 lg:p-5 min-h-screen w-full overflow-hidden">
            ...
            <dialog id="bulk_delete_selected" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <div class="mt-3 modal-action">
                        ...
                        <form method="POST" action="{{ route('admin.dashboard.order.transaction.bulk.delete-selected') }}"
                            @submit="loading = true">
                            @csrf
                            @method('DELETE')
                            <template x-for="id in $store.bulk.candidates" :key="id">
            ...
            <dialog id="bulk_delete_all" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <div class="modal-action">
                        ...
                        <form method="POST" action="{{ route('admin.dashboard.order.transaction.bulk.delete-all') }}"
                            @submit="loading = true">
                            @csrf
                            @method('DELETE')
                            <template x-for="id in $store.bulk.candidates" :key="id">
                        ...
                        @foreach ($transactions as $transaction)
                            <tr>
                                ...
                                <td>
                                    @if ($transaction['order_id'])
                                        <a href="{{ route('admin.dashboard.order.id.get', ['id' => $transaction['order_id']]) }}"
                                            class="cursor-pointer hover:underline">
                                            {{ $transaction['order']['order_number'] ?? 'No Invoice Found' }}
                                        </a>
                                    @else
                                        No Order Found
                                    @endif
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <div class="badge {{ $color }} border border-base-300 capitalize">
                                        {{ $transaction['status'] }}</div>
                                </td>
                                <td>{{ number_format($transaction['amount'], 2) }} {{ $site_currency }}</td>
                                <td x-cloak x-data
                                    x-show="$store.transaction_search_setting.showUpdatedTimeColumn || $store.transaction_search_setting.is_last_updated_filter">
                                ...
                                <div class="modal-box max-h-[85vh] max-w-2xl">
                                    ...
                                    <div class="text-sm space-y-2">
                                        ...
                                        <p><strong>Status:</strong> <span
                                                class="badge {{ $color }} border border-base-300">{{ ucfirst($transaction['status']) }}</span>
                                        </p>
