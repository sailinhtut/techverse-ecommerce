            ...
            <dialog id="bulk_delete_selected" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <div class="mt-3 modal-action">
                        ...
                        <form method="POST" action="{{ route('admin.dashboard.payment.payment.bulk.delete-selected') }}"
                            @submit="loading = true">
                            @csrf
                            @method('DELETE')
                            <template x-for="id in $store.bulk.candidates" :key="id">
            ...
            <dialog id="bulk_delete_all" class="modal" x-data="{ loading: false }">
                <div class="modal-box relative">
                    ...
                    <div class="modal-action">
                        ...
                        <form method="POST" action="{{ route('admin.dashboard.payment.payment.bulk.delete-all') }}"
                            @submit="loading = true">
                            @csrf
                            @method('DELETE')
                            <template x-for="id in $store.bulk.candidates" :key="id">
                    ...
                    <tbody>
                        @foreach ($payments as $payment)
                            <tr>
                                ...
                                <td x-cloak x-data x-show="$store.payment_search_setting.showIDColumn">
                                    {{ $payment['id'] }}
                                </td>
                                <td>
